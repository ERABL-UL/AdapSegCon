...
class MoCo(nn.Module):
    def __init__(self, model, model_head, dtype, args, K=65536, m=0.999, T=0.1):
        ...
        self.model_q = model(in_channels= 3, out_channels=latent_features[args.sparse_model])#.type(dtype)
        self.head_q = model_head(in_channels=latent_features[args.sparse_model], out_channels=args.feature_size)#.type(dtype)

        self.model_k = model(in_channels= 3, out_channels=latent_features[args.sparse_model])#.type(dtype)
